...
import ClientProgrammesPanel from '@/components/ClientProgrammesPanel'
...
export default function ClientDetailPage() {
  ...
  const [adherentTempPwd, setAdherentTempPwd] = useState('')
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Tab: Programmes — propulsé par PR v1 (ClientProgramme + status + dates + notes) */}
      {activeTab === 'programmes' && (
        <ClientProgrammesPanel clientId={params.id} />
      )}
          ...
          {client.adherentUserId ? (
            ...
            <div className="card p-6">
              ...
              <p className="text-sm text-surface-500 mb-5">
                ...
                Un mot de passe temporaire fort sera genere automatiquement, a transmettre a l&apos;adherent
                qui devra le changer a sa premiere connexion.
              </p>

              {/* Resultat creation : MdP affiche une seule fois */}
              {adherentTempPwd && (
                <div className="mb-5 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-3">
                  <div className="flex items-start gap-2.5">
                    ...
                    <p className="text-xs text-amber-700">
                      Mot de passe temporaire affiche <strong>une seule fois</strong>. Communiquez-le directement a l&apos;adherent.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => navigator.clipboard?.writeText(adherentTempPwd)}
                  ...
                  <button
                    type="button"
                    onClick={() => { setAdherentTempPwd(''); fetchData() }}
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    const res = await fetch(`/api/clients/${params.id}/account`, {
                      ...
                      body: JSON.stringify({ email: adherentEmail }),
                    })
                    const data = await res.json()
                    setAdherentSaving(false)
                    if (res.ok) {
                      setAdherentTempPwd(data.temporaryPassword || '')
                      setAdherentEmail('')
                    } else {
                      setAdherentError(data.error || 'Erreur')
                    }
                  }}
                  className="space-y-4"
                >
                  ...
                  <button type="submit" disabled={adherentSaving} className="btn-primary w-full justify-center">
                    {adherentSaving ? 'Creation...' : 'Generer le compte et le mot de passe'}
                  </button>
                </form>
              )}
            </div>
